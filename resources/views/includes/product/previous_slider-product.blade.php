												...
												<h4 class="price">{{ $prod->showPrice() }} <del><small>{{ $prod->showPreviousPrice() }}</small></del></h4>
														...
														<div class="item-cart-area">
															@if($prod->product_type == "affiliate")
																<button class="add-to-cart-btn affilate-btn"
																	data-href="{{ route('affiliate.product', $prod->slug) }}"><i class="icofont-cart"></i>
																	{{ $langg->lang251 }}
																</button>
															@else
																@if($prod->emptyStock())
																<button class="btn btn-primary  add-to-cart-btn cart-out-of-stock">
																	<i class="icofont-close-circled"></i> {{ $langg->lang78 }}
																</button>
																@else
																<button class="btn btn-primary  add-to-cart add-to-cart-btn" data-href="{{ route('product.cart.add',$prod->id) }}">
																	<i class="icofont-cart"></i> {{ $langg->lang56 }}
																</button>
																<button class="add-to-cart-quick add-to-cart-btn"
